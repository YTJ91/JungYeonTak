/* 
    생성자(constructor)
        - 객체에 대한 형태를 미리 만등러놓고 찍어서 생산할 수 있는 틀과 같은 개념.
        - 제품을 생산하기 위한 설계도 같은 역할.
        - 생성자 함수는 대문자로 시작한다.
...
//      => 객체를 만들기 쉽다.
//      => 단점은 같은 형태의 객채를 여러개 만들려면 번거롭다.
let user = {
  name: "seok",
  age: 30,
  hello: function () {
    // user의 이름과 나이를 포함한 인사말 출력하기.
...
/* 
    함수 내부에서 this를 통해서 값을 할당하는 생성자 함수가 된다.
        => this는 생성자 함수를 통해 생성될 인스턴스를 가르킨다.
        => return 값이 없어도 자동으로 인스턴스를 return 한다.
*/
...
// prototype : 인스턴스가 사용할 프로퍼티, 매서드를 상속시켜줄 수 있다.
...
// 정적 메서드 : 생성자 함수를 통해서 호출할 수 있는 매서드
...
/* 
    클래스(class)
        ...
        - 다른 프로그래밍 언어의 클래스 개념을 도입했다.
            ...
            => 문법적 설탕(Syntax Surgar)이라고 부른다
        -class 키워드와 함께 사용한다.
            => ()가 붙지않는다.
        -constructor를 통해 값을 초기화할 수 있다.
            => 초기화할 값이 없다면 contsructor를 생략할 수 있다.
            => 생략을 할 경우, JS가 내부적으로 자동으로 constructor를 실행한다. => 빈 객체를 만들어낸다.
        - 인스턴스 매서드는 함축표현으로 함수를 만들면 된다.
*/
...
// Animal 클래스 만들기
// sepcles(종):ex)포유류, 단공류, 양서류
// cry 매서드 만들기 => 울음소리 전달받아서 콘솔창에 출력.

class Animal {
  constructor(specles, legs) {
