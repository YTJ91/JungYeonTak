...
function sumTotal() {
  ...
  let argArr = Array.from(arguments); // Array.from() : 유사배열(반복가능한)을 배열로 만드는 정적 매서드.
  let result = 0; // 값을 초기화
  argArr.forEach((num) => {
    result += num; // 배열을 돌면서 각 숫자를 result에 산술 할당. => reduce로 대체 가능.
  });
  return result; // forEach에서 더한 후 return
}
console.log(sumTotal(1, 2, 3, 4, 5, 10));

// reduce 함수 활용해서 모든 인자 더하기.
// rest 문법 => 매개변수 앞에 ...을 붙이면 전달된 매개변수를 모두 배열로 모아준다.
//           => rest 매개변수는 가장 뒤에 와야한다.
function getTotal(...nums) {
  //   let rduArr = Array.from(arguments);
  //   return rduArr.reduce((acc, curr) => acc + curr, 0);
...
console.log(...numArr); // consloe.log(1,2,3,4,5). 값 5개를 펼힌 효과.
...
// 객체 스프레드 : 객체 속성들을 펼치는 효과. => 객체 안에다 펼쳐서 사용.
...
// obj 속성들을 속성 이름과 같은 이름의 변수로 할당하기.
// let name = obj.name;
// let age = obj.age;
// let foo = obj.foo;
...
let [first, second, third] = fruits; // 배열 비구조화 할당. let[] 안에 변수이름은 자유롭게 지으면 배열의 순서대로 할당한다.
