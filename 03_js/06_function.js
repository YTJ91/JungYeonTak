/* 
    ...
    매개 변수 (parameter)
        - 함수를 실행하는데 필요할 값을 호출 시점에 전달받는 변수.
            => 함수 내부에서 사용하는 변수처럼 사용.
            => 함수 내부에서만 유효한 범위를 가진다.
        - 매개 변수를 통해서 전달된 실제값을 인자(argument)라고 한다.
...
// 매개 변수가 없는 함수, 반환 값이 없는 함수. => 기본적으로 반복해서 사용할 코드를 작성.
function sum() {
  var num1 = 3;
  var num2 = 4;
...
console.log(square(3)); // 함수 호이스팅 발생 square 함수 선언문으로 작성했기 때문에 실행도 된다.
function square(num) {
  return num * num;
}
...
/* 
  콜백함수(callback)
    - 다른 함수에 인자로 전달되는 함수.
      => 함수의 실행결과를 가지고 다른 함수를 실행해야 할 때.
    - JS 에서는 함수를 값으로 평가하기 때문에 가능하다.
...
/* 
  sum03 사용해서 숫자 5, 7 를 더한 값을 7번 출력하기
...
/* 
  화살표 함수
    ...
    => 객체 return 시 ()로 객체를 감싸주어야 return을 생략할 수 있다.
    - 함수 호이스팅이 일어나지 않는다.
    - this 바인딩이 되지 않는다.
